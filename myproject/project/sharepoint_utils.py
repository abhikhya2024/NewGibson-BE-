...
SITE_PATH2 = "/sites/DocsFBTireMarin"
FOLDER = "FormattedQA"
TEXTFILESFOLDER = "OriginalFiles"
SITE_PATH1 = "/sites/DocsFarrarBallTireMFG"
...
def download_all_transcripts():
    ...
    # Step 1: Get Site ID
    site_res = requests.get(
        "https://graph.microsoft.com/v1.0/sites/cloudcourtinc.sharepoint.com:/sites/DocsFarrarBallTireMFG:/?select=id,webUrl",
...
def index_documents(ix, docs_list):
    ...
    for d in docs_list:
        writer.update_document(
            ...
            transcript_name_exact=d["transcript_name_exact"].lower(),
            transcript_name_search=normalize_index_text(d["transcript_name"]),

            witness_name=d["witness_name"],
            witness_name_search=normalize_index_text(d["witness_name"]),  # FIX HERE ✔
            ...
            project_name_search=normalize_index_text(d["project_name"])   # also FIX here
        )
    writer.commit()
...
def search_documents(ix, q_text_field_map, extra_filters=None, page=1, page_size=200, max_edits=1):
    results = []
    total_results = 0
    ...
    with ix.searcher() as searcher:
        ...
        combined_query = And(field_queries) if field_queries else Every()

        # Apply array filters (AND between them)
        if extra_filters:
            combined_query = And([combined_query] + extra_filters)

        try:
            page_obj = searcher.search_page(combined_query, page, pagelen=page_size)
            for hit in page_obj:
...
# def search_documents(ix, q_text_field_map, page=1,extra_filters=None, page_size=200, max_edits=1):
#     results = []
#     total_results = 0
#     extra_filters = extra_filters or []
